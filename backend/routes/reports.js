    ...
    if (lat && lng) {
      queryText = `
        ...
        WHERE moderation_status = 'approved'
          ...
          AND ST_DWithin(geom, ST_SetSRID(ST_MakePoint($2, $1), 4326)::geography, $5 * 1000)
        ORDER BY distance_km ASC
        LIMIT 500
      `;
      queryParams = [lat, lng, status || null, city || null, radius];
    } else {
      queryText = `
        SELECT id, lat, lng, address, city, district, photo_url,
               description, reporter_name, status, me_too_count, created_at
...
// POST /api/reports/:id/metoo — Ben de gördüm
router.post('/:id/metoo', param('id').isInt(), async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `UPDATE reports SET me_too_count = (SELECT COUNT(*) FROM me_too WHERE report_id = $1)
       WHERE id = $1`,
      [id]
    );
...
// POST /api/reports/:id/flag — uygunsuz içerik bildirimi
// 3+ şikayet → otomatik moderation_status = 'flagged' (admin kuyruğuna düşer)
router.post('/:id/flag', param('id').isInt(), async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      'SELECT COUNT(*) AS flag_count FROM report_flags WHERE report_id = $1',
      [id]
    );
